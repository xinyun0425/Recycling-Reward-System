...
<body>
    ...
    <div class="content">
        ...
        <div class="upperbutton">
            <button class="pending active">Pending Requests</button>
            <button class="assign" onclick="window.location.href='Admin-Pickup-Assign.php'">Assigned Requests</button>
            <button class="complete" onclick="window.location.href='Admin-Pickup-Complete.php'">Completed Requests</button>
        </div>
        <div class="detail-card">
            ...
            <?php
                ...
                $sql="SELECT pickup_request.pickup_request_id, user.username, pickup_request.datetime_submit_form, time_slot.date AS pickup_date, time_slot.time AS pickup_time, time_slot.no_driver_per_slot, pickup_request.status,
                    pickup_request.address,pickup_request.contact_no, pickup_request.remark, pickup_request.item_image, pickup_request.total_point_earned
                FROM pickup_request 
                ...
                WHERE pickup_request.status = 'unread'
                ORDER BY pickup_request.datetime_submit_form";
                $result=mysqli_query($con,$sql);
            ?>
            <div class="filter-container">
                <div class="filter-group">
                    <label for="filter_request_date">Filter by Request Date</label>
                    <input type="date" id="filter_request_date">
                </div>
                <div class="filter-group">
            ...
            <table id="request_table">
                ...
                <?php
                    ...
                    ?>
                        <tr class="row_hover" onclick="window.location.href='Admin-Pickup-Pending-Detail.php?pickup_request_id=<?php echo $row['pickup_request_id'];?>'">
                            <td><?php echo $counter++; ?></td>
                            <td><?php echo $row['username']; ?></td>
                            <td><?php echo date('d M Y', strtotime($row['datetime_submit_form'])); ?></td>
                    ...
                    ?>
                        <tr id="no-records-row">
                            <td colspan="6" style="text-align:center; padding: 20px; background-color:white;">
                                ...
                                No Pending Pickup Request Found.</span>
                            </td>
                        </tr>
                    <?php
                    }
    ...
    <script>
        ...
        function filterByDate() {
            const requestDate = document.getElementById('filter_request_date').value;
            const pickupDate = document.getElementById('filter_pickup_date').value;
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                const rowRequestDate = cells[2].textContent.trim();
                const rowPickupDate = cells[3].textContent.trim();

                const formattedRowRequestDate = formatDateForComparison(rowRequestDate.split(' ').slice(0, 3).join(' '));
                const formattedRowPickupDate = formatDateForComparison(rowPickupDate);

                let showRow = true;

                if (requestDate && formattedRowRequestDate !== requestDate) {
                    showRow = false;
                }

                if (pickupDate && formattedRowPickupDate !== pickupDate) {
                    showRow = false;
                }
