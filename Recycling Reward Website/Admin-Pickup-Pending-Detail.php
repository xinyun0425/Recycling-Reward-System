...
<style>
    ...
    .driver-select {
        ...
        font-size: 16px;
        appearance:none;
        background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="grey"><path d="M7 10l5 5 5-5z"/></svg>'); /* Custom dropdown arrow */
    ...
    .driver-details-hidden,.reject-details-hidden {
        display: none;
    }

    .driver-details-visible,.reject-details-visible{
        display: block;
        margin-top: 15px;
        padding: 12px 0px;
        border-radius: 6px;
...
<body>
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
            if ($_POST['action'] === 'assign_driver') {
                ...
                if (mysqli_query($con, $update_query)) {
                    ...
                    $pickup_query = "SELECT date, time FROM time_slot WHERE time_slot_id = '$time_slot_id'";
                    $pickup_result = mysqli_query($con, $pickup_query);
                    $pickup_data = mysqli_fetch_assoc($pickup_result);
                    $pickup_date = date('d M Y', strtotime($pickup_data['date']));
                    $pickup_time = date('H:i', strtotime($pickup_data['time']));

        
                    $announcement = "🚛 Great news! Your pickup request for $pickup_date at $pickup_time has been approved. Our driver, $driver_name, will arrive at your location during the scheduled time. If you need to get in touch, you can contact them at $contact_no. Please have your e-waste ready for collection. Thank you for recycling with us! ♻💚";
                    ...
                    $notificationQuery = "INSERT INTO user_notification(user_id, datetime, title, announcement, status) 
                                        VALUES ('$user_id', NOW(), 'Pickup Request Approved! 🚛 ', '$announcement', 'unread')";
                    ...
                    echo "<script>
                            ...
                            window.location.href='Admin-Pickup-Assign.php'; 
                        </script>";
                    exit();
                } else {
            ...
            if ($_POST['action'] === 'reject_request') {
                ...
                try {
                    ...
                    $user_result = mysqli_query($con, $user_query);
                    if (!$user_result) {
                        throw new Exception("User query failed: " . mysqli_error($con));
                    }
                    $user_data = mysqli_fetch_assoc($user_result);
                    $user_id = $user_data['user_id'];
                    ...
                    $announcement = "We're sorry! Your pickup request has been rejected due to the following reason: $reject_reason. You may submit a new request with updated details. If you have any questions, feel free to reach out. 🙏♻";
                    $announcement = mysqli_real_escape_string($con, $announcement);
                    $notificationQuery = "INSERT INTO user_notification(user_id, datetime, title, announcement, status) 
                                        VALUES ('$user_id', NOW(), 'Pickup Request Rejected ❌', '$announcement', 'unread')";
                    
                    if (!mysqli_query($con, $notificationQuery)) {
                        throw new Exception("Notification failed: " . mysqli_error($con));
                    }
    ...
    <div class="content">
        <h2 class='title'>
            <a href='Admin-Pickup-Pending.php' style='text-decoration: none; color: inherit;'>
            ...
            <div class="items-container">
                <div class="item-image-card">
                    <?php if(!empty($request['item_image'])): ?>
                        <iframe src="https://drive.google.com/file/d/<?= $request['item_image']; ?>/preview"></iframe>
                    <?php else: ?>
    ...
    <div class="pending-action">
        <button type="button" class="action-button assign-button" onclick="openAssignDriverModal()">
            <i class="fas fa-user-check"></i> Assign Driver
        </button>
        <button type="button" class="action-button reject-button" onclick="openRejectModal()">
            ...
            <div class="container">
                ...
                <div id="driverList">
                    ...
                    if (!empty($time_slot_id)) {
                        ...
                        if($available_slots > 0) {
                            $drivers = mysqli_query($con, "
                                SELECT driver.* 
                                FROM driver 
                                LEFT JOIN pickup_request 
                                    ON driver.driver_id = pickup_request.driver_id 
                            ...
                            if(mysqli_num_rows($drivers) > 0) {
                                ...
                                echo '<script>';
                                echo 'const driversData = ' . json_encode($drivers_data) . ';';
                                echo 'const pickupDate = "' . date('d M Y', strtotime($request['pickup_date'])) . '";';
                                echo 'const pickupTime = "' . date('H:i', strtotime($request['pickup_time'])) . '";';
                                echo '</script>';
                            ...
                            echo '<p><i class="fa-solid fa-circle-exclamation" aria-hidden="true" style="margin-right:5px; color: #c0bfbf; font-size:16px;"></i>No available slot for this time period.</p>';
                            $show_confirm_button = false;
                        }
                    } else {
                    ...
                    ?>
                        ...
                        <div id="driverText" class="driver-details-hidden">
                            <div class="driver-text">
                                <i class="fa fa-info-circle" aria-hidden="true" style="margin-right: 5px; color: #c0bfbf; font-size:20px;"></i>
                                <span id="driverDetailsText"></span>
                            </div>
                        </div>
                    </div>
                ...
                <?php if($show_confirm_button): ?>
                    <button type="submit" class="confirm-assign">Confirm</button>
                <?php else: ?>
                    <button type="button" class="confirm-assign" onclick="closeModal('assigndriver')">
                        <?php echo ($available_slots <= 0) ? 'OK' : 'Close'; ?>
                    </button>
                <?php endif; ?>
            </div>
        </form>
        ...
        <form class="modal-content animate" action="#" method="post">
            ...
            <div class="container">
                ...
                <select name="reject_reason" class="reject-reason-select" required >
                    ...
                    <option value="No available drivers">No available driver</option>
    ...
    <script>
        ...
        document.addEventListener("click", function(event) {
            ...
            const summaryText = document.getElementById("driverText");
        });

        function openAssignDriverModal() {
            document.getElementById('assigndriver').style.display = 'flex';
            
            const driverSelect = document.querySelector('select[name="driver_id"]');
            if (driverSelect) {
                driverSelect.addEventListener('change', updateSummaryText);
            }
        ...
        function openRejectModal() {
            document.getElementById('rejectRequest').style.display = 'flex';
            updateRejectSummary(); 
            body.style.overflow = "hidden";
        }
        ...
        function updateSummaryText() {
            ...
            const summaryText = document.getElementById("driverText");
            const detailsText = document.getElementById("driverDetailsText");
            
            if (!summaryText || !detailsText) return;
            
            if (!selectedDriverId) {
                summaryText.className = "driver-details-hidden";
                return;
            }
            
            if (typeof driversData !== 'undefined' && Array.isArray(driversData)) {
                const selectedDriver = driversData.find(driver => driver.driver_id == selectedDriverId);
                
                if (selectedDriver) {
                    detailsText.innerHTML = `
                        Driver <strong>${selectedDriver.driver_name}</strong> will be assigned for this pickup on 
                        <strong>${pickupDate}</strong> at <strong>${pickupTime}</strong>.
                    `;
                    summaryText.className = "driver-details-visible";
                    return;
                }
            }
